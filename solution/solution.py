...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        if not contexts:
            return 0.0
        relevant_flags = [len(_tokenize(chunk) & expected_tokens) / len(expected_tokens) >= relevance_threshold for chunk in contexts]
    ...
    def run_full_eval(
        ...
        return EvalResult(QAPair(question, expected, context), answer, faithfulness, relevance, completeness, passed, failure_type, context_precision, context_recall)
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            "mapping every criterion to a 0-1 score.\n\n"
            f"Question: {question}\nAnswer: {answer}\nRubric:\n{rubric_text}"
        )
        ...
        try:
            ...
            if not isinstance(raw_scores, dict):
                raise ValueError("Scores are not a mapping")
            scores = {
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        first_higher = len(averages) > 1 and averages[0] > sum(averages[1:]) / (len(averages) - 1)
        return {"positional_bias": first_higher, "leniency_bias": overall > 0.8, "severity_bias": overall < 0.3}
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            result = evaluator.run_full_eval(
                answer=agent_fn(pair.question), question=pair.question,
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        """
        def metric_average(items: list, metric: str) -> float:
            return sum(getattr(item, metric) for item in items) / len(items) if items else 0.0
        metrics = ("faithfulness", "relevance", "completeness")
        new_avgs = {metric: metric_average(new_results, metric) for metric in metrics}
        baseline_avgs = {metric: metric_average(baseline_results, metric) for metric in metrics}
        regressions = [metric for metric in metrics if baseline_avgs[metric] - new_avgs[metric] > 0.05]
        return {
            **{f"new_avg_{metric}": value for metric, value in new_avgs.items()},
            **{f"baseline_avg_{metric}": value for metric, value in baseline_avgs.items()},
    ...
    def identify_failures(
        ...
        """
        return [result for result in results if any(score < threshold for score in (result.faithfulness, result.relevance, result.completeness))]
...
class FailureAnalyzer:
    ...
    def generate_improvement_suggestions(
        ...
        suggestions_by_type = {
            "hallucination": "Implement a hallucination checker and require claims to be grounded in retrieved context",
            "irrelevant": "Improve prompt clarity with explicit intent handling and few-shot examples",
            "incomplete": "Increase context coverage and add generation guidance for complete answers",
            "off_topic": "Strengthen intent detection and add topic-boundary guardrails",
            "refusal": "Review guardrails to allow safe in-scope answers instead of unnecessary refusals",
        }
        suggestions = [suggestions_by_type.get(kind, "Review this failure cluster and add a targeted regression test") for kind in categories]
        defaults = [
            "Add representative failed cases to the golden dataset as regression tests",
            "Instrument retrieval and generation metrics to locate the weak pipeline stage",
            "Review failure samples with domain experts before deploying changes",
        ]
